...
import { categorize, codeOutput, concept, interactive, lesson, match, mcq, multi, numeric, order, shortAnswer, trueFalse } from '../builders';
      ...
      lessons: [
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'The kernel trick computes inner products in a higher-dimensional space without ever forming the coordinates.',
              true,
              ['sk-kernel-trick'],
              'It computes only the *dot products* that the projected coordinates would have produced. Never materialising the projection is precisely what makes an infinite-dimensional feature space tractable.',
            ),
          ],
        }),
...
print(gini(50, 50))`,
      ...
      checkpoint: {
        ...
        exercises: [
          ...
          trueFalse(
            'Random Forest trees can all be trained at the same time.',
            true,
            ['sk-ensembles'],
            'Each tree sees its own bootstrap sample and depends on no other tree, so the whole forest parallelises across cores for free. Gradient boosting cannot do this, because each of its trees is fitted to the previous ensemble’s errors.',
          ),
        ],
      },
    },
