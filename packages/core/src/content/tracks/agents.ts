...
export const agentsTrack: Track = {
  ...
  units: [
    ...
    // -----------------------------------------------------------------------
    {
      ...
      checkpoint: {
        id: 'checkpoint-agents-2',
        title: 'Tools and Reasoning Checkpoint',
        passingScore: 0.7,
        exercises: [
          mcq(
            'A model decides a tool is needed. What does it actually do?',
            [
              'Executes the function and returns the result',
              'Emits a structured request naming the tool and its arguments, which your code may then run',
              'Rewrites its own system prompt',
              'Pauses until a human approves',
            ],
            1,
            ['sk-tool-use'],
            'The model only ever produces a request. Execution, authorisation and auditing all happen in code you control — which is the only place they can be enforced.',
          ),
          trueFalse(
            'Chain-of-thought reasoning is a faithful record of how the model reached its answer.',
            false,
            ['sk-chain-of-thought'],
            'It is generated by the same next-token process as everything else and can rationalise rather than explain. Use it to improve answers and to debug; do not present it as an audit trail.',
          ),
          mcq(
            'What must an agent loop have besides a step limit?',
            [
              'A larger context window',
              'A budget cap and a detector for repeated no-progress actions',
              'A lower temperature',
              'A guarantee that it always produces a final answer',
            ],
            1,
            ['sk-react-pattern'],
            'A limit bounds the damage; a no-progress detector avoids it. Forcing a final answer is the trap — it converts an honest failure into a confident fabrication.',
          ),
        ],
      },
    },
