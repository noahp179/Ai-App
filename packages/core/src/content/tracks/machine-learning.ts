...
print(round(sigmoid(z), 2))`,
        ...
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'Scaling statistics should be computed on the training split alone, then applied to the others.',
              true,
              ['sk-feature-scaling', 'sk-data-leakage'],
              'Fitting the scaler on everything leaks the test set’s distribution into training, and the resulting score is optimistic in a way you only discover in production. Fit on train; transform validation and test with those same parameters.',
            ),
            concept(
              'Class imbalance',
      ...
      checkpoint: {
        ...
        exercises: [
          ...
          trueFalse(
            ...
            true,
            ['sk-logistic-regression'],
            'It fits a linear function and then squashes it through a sigmoid into a probability, which is thresholded into a class. The “regression” in the name refers to the linear part underneath, not to the output.',
          ),
        ],
      },
    },

    {
      ...
      checkpoint: {
        ...
        exercises: [
          ...
          trueFalse(
            'Gradient boosting trains its trees one after another, each correcting the last.',
            true,
            ['sk-ensembles'],
            'Each tree fits the residual errors of the ensemble so far, so tree n cannot start until tree n−1 is done. That sequential dependency is the price of its accuracy, and it is exactly what Random Forest gives up in exchange for trivial parallelism.',
          ),
        ],
      },
    },
