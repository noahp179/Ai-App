      ...
      lessons: [
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'An O(n) algorithm is always faster in practice than an O(n²) one.',
              false,
              ['sk-big-o'],
              'Only for sufficiently large n. Big-O says nothing about small inputs, where a large constant can dominate — which is exactly why production sort implementations switch to insertion sort below about 16 elements, despite insertion sort being O(n²).',
            ),
          ],
        }),
    ...
    return found`,
      ...
      checkpoint: {
        ...
        exercises: [
          ...
          trueFalse(
            'Amortised O(1) means every individual operation is O(1).',
            false,
            ['sk-amortised-analysis'],
            'Individual operations can be O(n). The guarantee is about the total across a sequence, which is what actually matters when you are doing many of them.',
          ),
          numeric(
      ...
      lessons: [
        ...
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'A heap keeps all of its elements in sorted order.',
              false,
              ['sk-heaps'],
              'Only the parent–child relationship is maintained; siblings are unordered. That weaker invariant is the point — it costs O(log n) to restore instead of O(n log n), and it is all you need when you only ever take from the front.',
            ),
          ],
        }),
      ],
