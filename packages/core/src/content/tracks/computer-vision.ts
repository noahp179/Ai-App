    ...
    // -----------------------------------------------------------------------
    {
      ...
      lessons: [
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'Feeding raw 0–255 pixel values to a model trained on normalised inputs fails silently.',
              true,
              ['sk-image-representation'],
      ...
      checkpoint: {
        ...
        exercises: [
          ...
          trueFalse(
            'A convolution kernel spans every channel of its input.',
            true,
            ['sk-convolution'],
            'A 3×3 kernel over a 64-channel feature map holds 3×3×64 weights and produces one output channel. Its width and height are local; its depth is not, which is why the parameter count grows with input channels rather than with image size.',
          ),
        ],
      },
    },
