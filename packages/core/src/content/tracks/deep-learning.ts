...
print(h.shape)`,
      ...
      checkpoint: {
        ...
        exercises: [
          ...
          trueFalse(
            'A network with only linear activations collapses into a single linear layer.',
            true,
            ['sk-activation-functions'],
            'Composing linear maps yields another linear map, so depth buys nothing at all without a non-linearity between the layers. XOR is the classic demonstration: no straight line separates its two classes, so no purely linear network of any depth can represent it.',
          ),
        ],
      },
    },
      ...
      lessons: [
        ...
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'Batch normalization uses batch statistics while training and running averages at inference.',
              true,
              ['sk-batch-norm'],
              'Two behaviours from one layer. Forget to switch the model to eval mode and it normalises by the statistics of whatever batch happens to be in front of it — a classic bug that produces mysteriously bad inference results.',
            ),
          ],
        }),
        ...
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'Dropout is switched off at inference time.',
              true,
              ['sk-dropout'],
              'Training only. At inference the full network is used, with activations scaled to match the expected magnitude seen during training — which is also why predictions are deterministic rather than varying run to run.',
            ),
            concept(
