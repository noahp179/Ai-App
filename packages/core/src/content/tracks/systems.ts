...
for j in range(n):
    for i in range(n):
        total += A[i][j]`,
            ...
            trueFalse(
              'Two algorithms with the same big-O complexity can differ by an order of magnitude in wall-clock time.',
              true,
              ['sk-memory-hierarchy', 'sk-big-o'],
              'Big-O counts operations and assumes each costs the same. Memory access costs vary by 50× within RAM and 250× more to disk, so access *pattern* routinely dominates operation count at identical complexity.',
            ),
          ],
        }),
      ...
      lessons: [
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'An async event loop provides concurrency without parallelism.',
              true,
              ['sk-async-io'],
              'Everything runs on a single thread, interleaved at `await` points. Nothing runs simultaneously — which is why an event loop never has data races, and why one blocking call freezes the whole thing.',
            ),
          ],
        }),
