  ...
  units: [
    {
      ...
      lessons: [
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'The same meaning can cost several times more tokens in one language than in another.',
              true,
              ['sk-tokenization'],
              'Tokenizers are trained on English-heavy corpora, so English encodes most efficiently. The same sentence in Thai, Hindi or Burmese can cost several times more tokens — meaning higher API cost and less usable context for those users.',
            ),
          ],
        }),
        ...
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'A model outputs a probability distribution over its whole vocabulary at each step.',
              true,
              ['sk-language-modeling'],
              'Sampling then picks one token from that distribution — often a subword fragment rather than a full word. The distribution is also what temperature and top-p reshape; they change how the pick is made, not what the model computed.',
            ),
            shortAnswer(
      ...
      checkpoint: {
        ...
        exercises: [
          ...
          trueFalse(
            'Embeddings are learned from data rather than hand-designed.',
            true,
            ['sk-embeddings'],
            'Their geometry falls out of the training objective — words used in similar contexts end up near each other because that is what minimises the loss. Nobody chooses what any individual dimension means, and in general no one can say.',
          ),
        ],
      },
    },
      ...
      lessons: [
        ...
        lesson({
          ...
          steps: [
            ...
            trueFalse(
              'Later messages in a chat cost more to process than earlier ones, not less.',
              true,
              ['sk-context-window'],
              'The model is stateless between requests, so the whole history is resent every turn and the prompt grows monotonically. Prompt caching reduces the cost of the repeated prefix but does not eliminate it.',
            ),
          ],
        }),
        lesson({
